...
class BarangkeluarController extends Controller
{
    ...
    public function proses_tambah(Request $request)
    {
        ...
        $jmlmasuk = BarangmasukModel::where('tbl_barangmasuk.barang_kode', $barang->barang_kode)
        ...
        $jmlkeluar = BarangkeluarModel::where('tbl_barangkeluar.barang_kode', $barang->barang_kode)
            ->when($request->tglawal, function ($query) use ($request) {
                return $query->whereBetween('bk_tanggal', [$request->tglawal, $request->tglakhir]);
            })->sum('tbl_barangkeluar.bk_jumlah');
    
        $stok_sekarang = $barang->barang_stok + ($jmlmasuk - $jmlkeluar);
    
        if ($stok_sekarang < $request->jml) {
            return response()->json(['error' => 'Jumlah stok tidak mencukupi'], 400);
        } else {
            // Jika stok mencukupi, tambahkan data barang keluar
